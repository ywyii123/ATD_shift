...
def solver(x_cur, x_lr, t_cur, t_next, net, second_order=False):
    t_cur = net.round_sigma(t_cur)
    t_next = net.round_sigma(t_next)
    denoised = net(x_cur, x_lr, t_cur).to(th.float64)
    d_cur = (x_cur - denoised) / t_cur
    x_next = x_cur + (t_next - t_cur) * d_cur
    if second_order:
        denoised = net(x_next, x_lr, t_next).to(th.float64)
...
def karras_schedule(
    ...
    steps = th.arange(num_timesteps, device=device) / max(num_timesteps - 1, 1)
...
def lognormal_timestep_distribution(
    ...
    [1] [Improved Techniques For Consistency Training](https://arxiv.org/pdf/2310.14189.pdf)
    """
    pdf = th.erf((th.log(sigmas[1:]) - mean) / (std * math.sqrt(2))) - th.erf(
        (th.log(sigmas[:-1]) - mean) / (std * math.sqrt(2))
    )
    pdf = pdf / pdf.sum()

    timesteps = th.multinomial(pdf, num_samples, replacement=True)
...
class GaussianDiffusion:
    ...
    def consistency_inference(self, y, model, first_stage_model, sigma_max, model_kwargs):
        z_y = self.encode_first_stage(y, first_stage_model, up_sample=True)
        sigma = th.Tensor([sigma_max]).repeat(y.shape[0], 1, 1, 1).to(y.device)

        noise = th.rand_like(z_y)
        z_T = z_y + noise * sigma 
        pred_z0 = model(z_T, sigma, lq=z_y).detach()
        pred_x0 = self.decode_first_stage(pred_z0, first_stage_model,)
        return pred_x0
    ...
    def training_losses(
            self, model, x_start, y, current_iters, total_iters, rho, s0, s1, sigma_min, sigma_max, P_mean, P_std, writer, loss_type,
            first_stage_model=None,
            model_kwargs=None,
            noise=None,
            lpips_loss=None,
        ...
        e0 = z_y - z_start
        num_steps = improved_timesteps_schedule(current_iters, total_iters, s0, s1)
        num_steps = int(num_steps)
        sigmas = karras_schedule(num_steps, sigma_min, sigma_max, rho, x_start.device)
        if P_mean == 0 and P_std == 0:
            timestep = th.randint(0, num_steps - 1, size=(1,), device=x_start.device)
        else:
            timestep = lognormal_timestep_distribution(1, sigmas, P_mean, P_std)
        
        timestep = timestep.repeat(x_start.shape[0])
        current_sigma = sigmas[timestep].reshape([x_start.shape[0], 1, 1, 1])
        next_sigma = sigmas[timestep + 1].reshape([x_start.shape[0], 1, 1, 1])
        
        z_t = z_start + e0 * current_sigma / sigma_max + current_sigma * noise
        z_next = z_start + e0 * next_sigma / sigma_max + next_sigma * noise
        terms = {}
        terms["loss"] = 0
        with th.no_grad():
            distiller_target = model(z_t, current_sigma, lq=z_y).detach()
            # distiller_target = z_start
        distiller = model(z_next, next_sigma, lq=z_y)
        weights = improved_loss_weighting(sigmas)[timestep].reshape([x_start.shape[0], 1, 1, 1])
        if 'l1' in loss_type:
            terms["l1"] = (distiller - distiller_target).abs() * weights
            terms["loss"] += terms["l1"]
        elif 'l2' in loss_type:
            terms["l2"] = ((distiller - distiller_target) ** 2) * weights
            terms["loss"] += terms["l2"]
        elif 'huber' in loss_type:
            terms["huber"] = pseudo_huber_loss(distiller, distiller_target) * weights            
            terms["loss"] += terms["huber"]
        elif 'lpips' in loss_type:
            # lpips_loss = lpips.LPIPS(net='vgg').cuda()
            terms["lpips"] = lpips_loss(distiller, distiller_target) * weights
            terms["TV"] = TV_loss(distiller)
            terms["loss"] += (terms["lpips"] + terms["TV"])
        
        pred_z0 = distiller.detach()
        pred_z0_target = distiller_target
        pred_x0 = self.decode_first_stage(pred_z0, first_stage_model)
        pred_x0_target = self.decode_first_stage(pred_z0_target, first_stage_model)
        pred_x0_tensor = (pred_x0 + 1.0) / 2
        pred_x0_target_tensor = (pred_x0_target + 1.0) / 2
        pred_x0_tensor = pred_x0_tensor.clamp(0, 1)
        pred_x0_target_tensor = pred_x0_target_tensor.clamp(0, 1)
        x0_tensor = (x_start + 1.0) / 2
        x0_tensor = x0_tensor.clamp(0, 1)
        x_cur = ((self.decode_first_stage(z_t, first_stage_model) + 1.0) / 2).clamp(0, 1)
        x_next = ((self.decode_first_stage(z_next, first_stage_model) + 1.0) / 2).clamp(0, 1)
        if dist.get_rank() == 0 and current_iters % 100 == 0:
            writer.add_scalar('loss', terms["loss"].mean(), global_step=current_iters)
            writer.add_images('distiller', pred_x0_tensor, global_step=current_iters, dataformats='NCHW')
            writer.add_images('x_next', x_next, global_step=current_iters, dataformats='NCHW')
            writer.add_images('distiller_target', pred_x0_target_tensor, global_step=current_iters, dataformats='NCHW')
            writer.add_images('x_cur', x_cur, global_step=current_iters, dataformats='NCHW')
            writer.add_images('x0', x0_tensor, global_step=current_iters, dataformats='NCHW')
