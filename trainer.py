...
from models.losses import WeightedTVLoss
...
from basicsr.utils.logger import (AvgTimer, MessageLogger, get_env_info, get_root_logger,
...
class TrainerBase:
    def __init__(self, configs):
        ...
        # setup seed
        self.setup_seed()
        self.writer = SummaryWriter(self.configs.consistency.log_path)
    ...
    def init_logger(self):
        ...
        if self.rank == 0:
            ...
            # self.logger = get_root_logger(logger_name='basicsr', log_level=logging.INFO, log_file=logtxet_path)
            self.logger = logger
            self.logger.remove()
    ...
    def train(self):
        ...
        for ii in range(self.iters_start, self.configs.train.iterations):
            ...
            # training phase
            self.training_step(data, self.current_iters, total_iters)

            # validation phase
            if 'val' in self.dataloaders and (ii+1) % self.configs.train.get('val_freq', 10000) == 0:
                val_freq = self.configs.train.get('val_freq', 10000)
                tick_end_time = time.time()
                self.logger.info(f"sec/iter:{((tick_end_time - tick_start_time) / val_freq):<7.2f}")
                self.validation()
                tick_start_time = time.time()
...
class TrainerDifIR(TrainerBase):
    ...
    def build_model(self):
        ...
        # autoencoder
        print(self.configs.autoencoder)
        ...
        self.TV_loss = WeightedTVLoss().cuda()
    ...
    def training_step(self, data, current_iters, total_iters):
        ...
        for jj in range(0, current_batchsize, micro_batchsize):
            ...
            rho = self.configs.consistency.params.rho
            s0 = self.configs.consistency.params.s0
            s1 = self.configs.consistency.params.s1
            sigma_min = self.configs.consistency.params.sigma_min
            sigma_max = self.configs.consistency.params.sigma_max
            P_mean = self.configs.consistency.params.P_mean
            P_std = self.configs.consistency.params.P_std
            loss_type = self.configs.consistency.params.loss_type
    ...
    def validation(self, phase='val'):
        if self.rank == 0:
            ...
            for ii, data in enumerate(self.dataloaders[phase]):
                ...
                #     tt -= 1
                sample_decode={}
                sample_decode['sample'] = self.base_diffusion.consistency_inference(
                        y=im_lq,
                        model=self.ema_model if self.configs.train.use_ema_val else self.model,
                        first_stage_model=self.autoencoder,
                        model_kwargs=model_kwargs,
                ...
                if 'gt' in data:
                    ...
                    mean_lpips += self.lpips_metric(sample_decode['sample'].detach(), im_gt).sum().item()

                if (ii + 1) % self.configs.train.log_freq[2] == 0:
                    # self.logger.info(f'Validation: {ii+1:02d}/{num_iters_epoch:02d}...')
                    # sr_tensor = (sample_decode['sample'].detach() * 0.5) + 0.5
                    # util_image.imwrite(
                    #     sr_tensor.cpu().squeeze(0).permute(1, 2, 0).numpy(),
                        # self.image_dir / phase / f"{ii + 1}.png"
                    # )
                    def save_image(img):
                        image = img.squeeze(0).cpu().float().clamp_(-1, 1).numpy()
                        img = np.transpose(image[[2, 1, 0], :, :], (1, 2, 0))
                        img = (img * 127.5 + 128).clip(0, 255).astype(np.uint8)
                        return img
                    val_save_path = os.path.join(self.image_dir / phase, f'validation-{self.current_iters:06d}')
                    path = val_save_path / f"{ii + 1}.png"
                    cv2.imwrite(str(path), save_image(sample_decode['sample'].detach()))
            ...
            if 'gt' in data:
                ...
                self.logger.info(f'Validation Metric: PSNR={mean_psnr:5.2f}, LPIPS={mean_lpips:6.4f}, iteration={self.current_iters}')

                if dist.get_rank() == 0:
                    print(f'Validation: PSNR={mean_psnr:5.2f}, LPIPS={mean_lpips:6.4f}')
