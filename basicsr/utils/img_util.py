...
def generate_lq(gt, upscale,):
    # upscale = int(upscale)
    gt = (gt + 1.0) / 2
    h_ori, w_ori = gt.shape[-2:]
    gt = gt.permute(0, 2, 3, 1).cpu().numpy()
    h, w = math.ceil(h_ori / upscale), math.ceil(w_ori / upscale)
    gt_imgs = []
    if h * upscale > h_ori or w * upscale > w_ori:
        pad_h = int(max(h * upscale - h_ori, 0))
        pad_w = int(max(w * upscale - w_ori, 0))
    ...
    lq_imgs = []
    for img in gt_imgs:
        lq = cv2.resize(img, dsize=None, fx=1/upscale, fy=1/upscale, interpolation=cv2.INTER_CUBIC)
        lq_imgs.append(lq)
    lq_imgs = np.array(lq_imgs)
    lq_imgs = torch.from_numpy(lq_imgs.transpose(0, 3, 1, 2))
    lq_imgs = lq_imgs * 2 - 1
