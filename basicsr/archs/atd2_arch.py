...
class WindowAttention(nn.Module):
    ...
    def forward(self, qkv, rpi, mask=None):
        ...
        qkv = qkv.reshape(b_, n, 3, self.num_heads, c // self.num_heads).permute(2, 0, 1, 3, 4)
        ...
        # print('rpb: ', relative_position_bias.shape)
        # if mask is not None:
        #     print('mask: ', mask.shape)
        x = xformers.ops.memory_efficient_attention(q, k, v, relative_position_bias.expand(b_, -1, -1, -1))
...
class AC_MSA(nn.Module):
    ...
    def forward(self, qkv, sim, x_size):
        ...
        qkv = y.reshape(b * ng, gs, 3, self.num_heads, c//self.num_heads).permute(2, 0, 1, 3, 4)  # 3, b*ng, gs, nh, c//nh
        ...
        y = xformers.ops.memory_efficient_attention(q, k, v, scale=logit_scale)
        ...
        y = y.permute(0, 2, 1, 3).reshape(b, n+pad_n, c)[:, :n, :]
