...
class ATD(nn.Module):
    ...
    def __init__(self,
        ...
        elif self.upsampler == 'consistency':
            self.conv_first = nn.Conv2d(num_in_ch, embed_dim, self.upscale, self.upscale,)
            self.conv_before_upsample = nn.Sequential(
                nn.Conv2d(embed_dim, num_feat, 3, 1, 1), nn.LeakyReLU(inplace=True))
            self.upsample = Upsample(upscale, num_feat)
            self.conv_last = nn.Conv2d(num_feat, num_out_ch, 3, 1, 1)
    ...
    def forward(self, x, timesteps, lq=None):
        #time_embed
        sigma = timesteps.to(torch.float32).reshape(-1, 1, 1, 1)
        ...
        elif self.upsampler == 'denoising':
            # for image denoising and JPEG compression artifact reduction
            x_first = self.conv_first(x)
            res = self.conv_after_body(self.forward_features(x_first, params)) + x_first
            x = x + self.conv_last(res)
        elif self.upsampler == 'diffusion':
            x_first = self.conv_first(x)
            x = self.conv_after_body(self.forward_features(x_first, params, emb=emb))
            x = self.conv_last(x)
        elif self.upsampler == 'consistency':
            
            x_first = self.conv_first(x)
            x = self.conv_after_body(self.forward_features(x_first, params, emb=emb)) + x_first
            x = self.conv_before_upsample(x)
            x = self.conv_last(self.upsample(x))
            c_skip = self.sigma_data ** 2 / (((sigma / 0.1)) ** 2 + self.sigma_data ** 2)
            c_out = (sigma / 0.1) / ((sigma / 0.1) ** 2 + self.sigma_data ** 2).sqrt()
            x = c_skip * x_t + c_out * x.to(torch.float32)
            # print(x.shape)
