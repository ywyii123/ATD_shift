    ...
    def forward(self, x, timestep, sigma, lq=None, gt_size=None):
        ...
        if self.block_type == 'swin':
            ...
            if lq is not None and self.cond_lq:
                upscale = self.up_list[timestep]
                downscale = self.down_list[timestep]
                h_lq = h * downscale // upscale
                w_lq = w * downscale // upscale
            ...
            if lq is not None and self.cond_lq:
                ...
                h_lq = h * downscale // upscale
                w_lq = w * downscale // upscale
                lq = torch.cat([lq, torch.flip(lq, [2])], 2)[:, :, :h_lq, :]
                lq = torch.cat([lq, torch.flip(lq, [3])], 3)[:, :, :, :w_lq]
                # x = torch.cat([lq, x], dim=1)
        # print(f"x_shape:{x.shape}")
        # print(f"lq_shape:{lq.shape}")
        x_first = encoder(x, lq=lq)
        x_next = x_first
...
class Encoder(nn.Module):
    def __init__(
        ...
        if cond_lq:
            in_chans = in_chans * 2
        if upscale != 1:
            self.conv_before_up = nn.Conv2d(in_chans, in_chans * (upscale ** 2), 3, 1, 1)
            self.upsample = nn.PixelShuffle(upscale)
        else:
            self.upsample = torch.nn.Identity()
            self.conv_before_up = nn.Conv2d(in_chans, in_chans, 3, 1, 1)
        self.downsample = nn.PixelUnshuffle(downscale)
        
        num_in_chns = in_chans * (downscale ** 2)
        # if cond_lq:
        #     num_in_chns += 3
