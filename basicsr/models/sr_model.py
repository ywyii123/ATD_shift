...
from basicsr.utils import improved_timesteps_schedule, karras_schedule, lognormal_timestep_distribution, improved_loss_weighting, pseudo_huber_loss, q_sample
    ...
    def optimize_parameters(self, current_iter, tb_logger=None, log_iter=None):
        ...
        if self.cri_consistency:
            ...
            if p_mean == 0 and p_std == 0:
                ...
                timestep = timestep.repeat(self.gt.shape[0])
            else :
                timestep = lognormal_timestep_distribution(1, sigmas, p_mean, p_std)
                timestep = timestep.repeat(self.gt.shape[0])
            noise = torch.randn_like(self.gt)
            cur_sigma = sigmas[timestep].reshape(self.gt.shape[0], 1, 1, 1)
            next_sigma = sigmas[timestep + 1].reshape(self.gt.shape[0], 1, 1, 1)
            x_cur = q_sample(self.gt, self.lq, sigmas, timestep, noise)
            x_next = q_sample(self.gt, self.lq, sigmas, timestep + 1, noise)
            with torch.no_grad():
                distiller_target = self.net_g(x_cur, cur_sigma, self.lq).detach()
            distiller = self.net_g(x_next, next_sigma, self.lq)
            ...
            if dist.get_rank() == 0 and current_iter % log_iter == 0:
                ...
                tb_logger.add_images('lq', lq_tensor, global_step=current_iter, dataformats='NCHW')
