...
from basicsr.utils.diffusion import karras_schedule, p_sample_upsample_loop, p_sample_upsample_loop_one_backbone
    ...
    def test(self, gt_size=None):
        ...
        if multi_step == False:
            ...
            if hasattr(self, 'net_g_ema'):
                self.net_g_ema.eval()
                with torch.no_grad():
                    # self.output = p_sample_upsample_loop_one_backbone(self.lq,
                    #                                                   self.net_g_ema, 
                    #                                                   num_steps=num_steps, 
                    #                                                   sigmas=sigmas, 
                    #                                                   up_list=up_list, 
                    ...
                    self.output = p_sample_adaptive(self.lq,
                                                    self.net_g_ema, 
                                                    num_steps=num_steps, 
                                                    sigmas=sigmas, 
                                                    up_list=up_list, 
                                                    down_list=down_list, 
                                                    gt_size=gt_size,
                                                    input_scale_list=input_scale_list
                                                    )
            else:
                self.net_g.eval()
                with torch.no_grad():
